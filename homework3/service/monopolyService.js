...
router.get("/playerGame", readPlayerGame); // Code added by NW on Nov 7
...
function readPlayerGame(req, res, next) {
    db.many("SELECT score FROM Player, PlayerGame WHERE Player.ID = PlayerGame.playerID AND Player.name = 'Luigi';")
        .then(data => {
            res.send(data);
        })
        .catch(err => {
